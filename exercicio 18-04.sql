...
/*Selecione todas as colunas da tabela de Obrigacao onde o favorecido é “CELESC SA” e a coluna
estaPago é falso (resultado, 1 linha).*/
SELECT * FROM financeiro.obrigacao WHERE idFavorecido = 1 AND estaPago = false;
/*Inserir na tabela Obrigacao um registro com Data de vencimento 25/09/2015, valor de 100,00, código
de barras vazio, favorecido “Embracon Seguranca e Vigilancia Ltda”, conta “BB Principal”, categoria
“Serviços”.*/
...
/*Atualizar todas as obrigações do favorecido “Walmart”, acrescentando um mês a data de vencimento (resultado, 2 linhas).*/
/*SELECT idObrigacao, dataVencimento, date_add(DataVencimento, interval 1 month) FROM financeiro.obrigacao WHERE idFavorecido = 2;*/
UPDATE financeiro.obrigacao SET dataVencimento = date_add(DataVencimento, interval 1 month) WHERE idFavorecido = 2;
/*Para a data de vencimento não cair em feriado, sabado e domingo*/
